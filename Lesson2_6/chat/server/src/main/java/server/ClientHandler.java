            ...
            new Thread(() -> {
                try {
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/reg ")) {
                            ...
                            if (!b) {
                                ...
                                logger.log(Level.WARNING,"Ошибка: с этим логином уже Зарегистированы.");
                        ...
                        if (str.startsWith("/auth ")) {
                            ...
                            if (newNick != null) {
                                if (!server.isLoginAuthorized(token[1])) {
                                    ...
                                    logger.log(Level.WARNING,"С этим логином уже авторизовались");

                                }
                            } else {
                                sendMsg("Неверный логин / пароль");
                                logger.log(Level.WARNING,"Неверный логин / пароль");
                    ...
                    while (true) {
                        String str = in.readUTF();
                        if (str.startsWith("/")) {
                            logger.log(Level.INFO,"Клиент послал команду "+str);
                            if (str.equals("/end")) {
                                sendMsg("/end");
                                break;
                            }
                            ...
                            if (str.startsWith("/chnick ")) {
                                ...
                                if (server.getAuthService().changeNick(this.nick, token[1])) {
                                    ...
                                    logger.log(Level.INFO,"Ваш ник изменен на " + token[1]);
                                    this.nick = token[1];
                                    server.broadcastClientlist();
                                } else {
                                    sendMsg("Не удалось изменить ник. Ник " + token[1] + " уже существует");
                                    logger.log(Level.WARNING,"Не удалось изменить ник. Ник " + token[1] + " уже существует");

                                }
                            }
                        } else {
                            server.broadcastMsg(nick, str);
                            logger.log(Level.INFO,"Клиент послал сообщение "+str);
